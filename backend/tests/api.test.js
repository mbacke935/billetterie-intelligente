...
describe('Tests d\'intégration de l\'API - Billetterie Intelligente', () => {
    ...
    beforeAll(async () => {
        ...
        // Création et démarrage de l'instance MongoDB en mémoire
        mongod = await MongoMemoryServer.create();
        // Récupération de l'URI de connexion générée pour le serveur en mémoire
        const uri = mongod.getUri();

        // Attribution de l'URI de la base en mémoire à la variable d'environnement MONGO_URI
        process.env.MONGO_URI = uri;

        // Connexion de Mongoose à la base de données en mémoire
        await mongoose.connect(uri);

        // Chargement de l'application Express après la configuration de MONGO_URI
        app = require('../server');
